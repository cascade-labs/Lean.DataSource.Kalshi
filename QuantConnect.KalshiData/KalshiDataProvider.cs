...
namespace QuantConnect.Lean.DataSource.KalshiData
{
    ...
    /// </summary>
    public class KalshiDataProvider : SynchronizingHistoryProvider, IDataQueueHandler
    {
        ...
        private KalshiDataRestClient? _restApiClient;
        ...
        private void Initialize()
        {
            ...
            Log.Trace("KalshiDataProvider: Initializing...");
            ...
            _restApiClient = new KalshiDataRestClient(_rateGate);
            _symbolMapper = new KalshiSymbolMapper();

            _initialized = true;

            Log.Trace("KalshiDataProvider: Initialized successfully");
        }
        ...
        public IEnumerable<BaseData>? GetHistory(HistoryRequest historyRequest)
        {
            ...
            {
                Log.Error("KalshiDataProvider: Not initialized");
                return null;
            }
            ...
            if (!SupportedResolutions.Contains(historyRequest.Resolution))
            {
                if (!_invalidResolutionWarningFired)
                {
                    ...
                    Log.Trace($"KalshiDataProvider: Unsupported resolution '{historyRequest.Resolution}'. Only Minute is supported.");
                }
                return null;
            }
            ...
            if (historyRequest.TickType != TickType.Quote && historyRequest.TickType != TickType.Trade)
            {
                if (!_invalidTickTypeWarningFired)
                {
                    ...
                    Log.Trace($"KalshiDataProvider: Unsupported tick type '{historyRequest.TickType}'. Only Quote/Trade supported.");
                }
                return null;
            }
            ...
            {
                Log.Error($"KalshiDataProvider: Invalid Kalshi symbol '{symbol.Value}'");
                return null;
            }
            ...
            Log.Trace($"KalshiDataProvider.GetHistory: Symbol={symbol.Value}, Ticker={ticker}, Resolution={historyRequest.Resolution}");
            Log.Trace($"KalshiDataProvider: Fetching {ticker} from {startTimeLocal:yyyy-MM-dd HH:mm} to {endTimeLocal:yyyy-MM-dd HH:mm} ET");
            ...
            Log.Trace($"KalshiDataProvider: Returning {result.Count} bars for {ticker}");
            return result;
        }
        ...
        public IEnumerator<BaseData>? Subscribe(SubscriptionDataConfig dataConfig, EventHandler newDataAvailableHandler)
        {
            ...
            Log.Trace($"KalshiDataProvider: Live subscription not supported for {dataConfig.Symbol}");
            return null;
        }
